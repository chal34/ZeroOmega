...
describe('RuleList', () => {
  describe('AutoProxy', () => {
    ...
    it('should parse keyword conditions', () => {
      ...
      expect(result[0]).toEqual({
        source: line,
        profileName: 'match',
        condition: { conditionType: 'KeywordCondition', pattern: 'example.com' },
      })
    })
    it('should parse keyword conditions with asterisks', () => {
      ...
      expect(result[0]).toEqual({
        source: line,
        profileName: 'match',
        condition: { conditionType: 'UrlWildcardCondition', pattern: 'http://*example*.com*' },
      })
    })
    ...
    it('should parse "starts-with" conditions', () => {
      ...
      expect(result[0]).toEqual({
        source: line,
        profileName: 'match',
        condition: { conditionType: 'UrlWildcardCondition', pattern: 'https://ssl.example.com*' },
      })
    })
  ...
  describe('Switchy', () => {
    ...
    const compose = (sections: Record<string, string[]>) => {
      let list = '#BEGIN\r\n\r\n'
    ...
    it('should parse wildcard rules', () => {
      ...
      expect(result[0]).toEqual({
        ...
        profileName: 'match',
        condition: { conditionType: 'UrlWildcardCondition', pattern: '*://example.com/abc/*' },
      })
    })
    ...
    it('should parse multiple rules in multiple sections', () => {
      ...
      expect(result[0]).toEqual({
        source: 'http://www.example.com/*',
        profileName: 'match',
        condition: { conditionType: 'UrlWildcardCondition', pattern: 'http://www.example.com/*' },
      })
      expect(result[1]).toEqual({
  ...
  describe('Switchy (omega format)', () => {
    ...
    it('should compose and parse disabled rules', () => {
      const rule = {
        ...
        condition: { conditionType: 'FalseCondition', pattern: '*.example.com' },
        profileName: 'match',
      }
      const list = compose({ rules: [rule], defaultProfileName: 'notmatch' })
      const result = parse(list, 'match', 'notmatch')
    ...
    it('should compose and parse conditions starting with special chars', () => {
      const rule = {
        source: ': ;abc',
        condition: { conditionType: 'HostWildcardCondition', pattern: ';abc' },
        profileName: 'match',
      }
      const list = compose({ rules: [rule], defaultProfileName: 'notmatch' })
      const result = parse(list, 'match', 'notmatch')
    ...
    it('should parse multiple conditions', () => {
      const rules = [
        ...
        {
          source: '*.example.org',
          condition: { conditionType: 'HostWildcardCondition', pattern: '*.example.org' },
          profileName: 'match',
        },
      ]
      ...
      const rules = [
        {
          ...
          condition: { conditionType: 'HostWildcardCondition', pattern: 'b.example.com' },
          profileName: 'match',
        },
        {
          source: '!a.example.org',
          condition: { conditionType: 'HostWildcardCondition', pattern: 'a.example.org' },
          profileName: 'notmatch',
        },
      ]
      const list = compose({ rules, defaultProfileName: 'notmatch' })
