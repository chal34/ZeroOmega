      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title">{{ tr('options_modalHeader_applyOptions') }}</h4>
          </div>
          <div class="modal-body">
            <p>{{ tr('options_applyOptionsRequired') }}</p>
            <p>{{ tr('options_applyOptionsConfirm') }}</p>
          </div>
          <div class="modal-footer">
            <button @click="emit('cancel')" class="btn btn-default">{{ tr('dialog_cancel') }}</button>
            <button type="button" @click="emit('confirm')" class="btn btn-primary">{{ tr('options_apply') }}</button>
          </div>
        </div>
      </div>
    </div>
